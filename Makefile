...
SUPPORTED_COMMANDS := console composer phpunit compass bower cap capifony
...
console:
	@docker run -ti --rm=true \
		...
		--link supratecplateformeachat_db_1:db \
		--link supratecplateformeachat_rabbitmq_1:rabbitmq \
		supratecplateformeachat_php bash -ci '\
			$(CREATE_USER_COMMAND) \
			$(ADD_SSH_ACCESS_COMMAND) \
			cd /srv/http && $(EXECUTE_AS) php app/console $(COMMAND_ARGS)'
...
cap:
	@docker run -ti --rm=true \
		-v `pwd`:/root/workdir \
		-v $(shell readlink -f $(SSH_AUTH_SOCK)):/root/ssh-agent \
		mjanser/capifony $(COMMAND_ARGS)
